...
public class IdCardVerify {
     ...
     * @return
     */
    public static void IdentityCardVerification(String idStr){
        String[] wf = { "1", "0", "x", "9", "8", "7", "6", "5", "4", "3", "2" };
        String[] checkCode = { "7", "9", "10", "5", "8", "4", "2", "1", "6", "3", "7", "9", "10", "5", "8", "4", "2" };
        String iDCardNo = "";
        //判断号码的长度 15位或18位
        if (idStr.length() != 15 && idStr.length() != 18) {
            String msg =  "身份证号码长度应该为15位或18位";
            log.error("【证件错误】,{}，{}",msg,idStr);
        ...
        if (isStrNum(iDCardNo) == false) {
            String msg =  "身份证15位号码都应为数字;18位号码除最后一位外,都应为数字";
            log.error("【证件错误】,{}，{}",msg,idStr);
            throw new RuntimeException(msg);
        }
        //判断出生年月
